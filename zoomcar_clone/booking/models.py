...
class Booking(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.price = (1000 * self.duration ) * (1 - self.discount/100)

        if not self.vehicle_registration_id:
            self.vehicle_registration_id = "".join(choices(ascii_uppercase, k=2)) + "".join(choices(
                digits, k=2)) + " " + "".join(choices(ascii_uppercase, k=1)) + " " + "".join(choices(digits, k=4))
